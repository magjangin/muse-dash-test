...
{
    /// <summary>
    /// 세이브 파일 오염 방지를 위해 가상 앨범/곡 기록(9999-x, 9998-x)이 디스크에 물리적으로 저장되기 직전
    ...
    public static class SaveDataManagerPatch
    {
                ...
                foreach (var k in datas.Keys)
                {
                    ...
                    if (subData != null)
                    {
                        ...
                        if (fields != null)
                        {
                            ...
                            {
                                if (key != null && (key.StartsWith("9999-") || key.StartsWith("9998-")))
                                {
        ...
        private static int CleanIDataList(Il2CppSystem.Collections.Generic.List<IData> list)
        {
            ...
            for (int i = list.Count - 1; i >= 0; i--)
            {
                ...
                if (songResult != null)
                {
                    ...
                    if (uid != null && (uid.StartsWith("9999-") || uid.StartsWith("9998-")))
                    {
                        list.RemoveAt(i);
                        removedCount++;
                        continue;
        ...
        private static int CleanStringList(Il2CppSystem.Collections.Generic.List<string> list)
        {
            ...
            {
                string val = list[i];
                if (val != null && (val.StartsWith("9999-") || val.StartsWith("9998-")))
                {
                    list.RemoveAt(i);
                    removedCount++;
                }

...
public class PnlStage_ChangeMusic_Patch
{
    public static void Postfix(PnlStage __instance, int i)
    {
        try
        {
            MelonLogger.Msg($"[PnlStage.ChangeMusic] enter index={i}, previousSelectedUid={CustomPlaySession.Current.SelectedMusicUid}, previousShouldApply={CustomPlaySession.Current.ShouldApplyExperimentChart}");
            PnlStagePatchHelper.SyncExperimentModeFromStage(__instance);
            ...
            MelonLogger.Msg($"[PnlStage.ChangeMusic] exit index={i}, selectedUid={CustomPlaySession.Current.SelectedMusicUid}, currentShouldApply={CustomPlaySession.Current.ShouldApplyExperimentChart}");
        }
...
public class PnlStage_RefreshDiffUI_Patch
{
    ...
    public static void Postfix(PnlStage __instance, Il2CppAssets.Scripts.Database.MusicInfo musicInfo)
    {
        try
        {
            MelonLogger.Msg($"[PnlStage.RefreshDiffUI.Postfix] enter uid={musicInfo?.uid ?? "(null)"}, currentSelectedUid={CustomPlaySession.Current.SelectedMusicUid}, shouldApply={CustomPlaySession.Current.ShouldApplyExperimentChart}");
            CustomRecordUiPatchHelper.ApplyCustomRecordToPnlStage(__instance, musicInfo);
            ...
            MelonLogger.Msg($"[PnlStage.RefreshDiffUI.Postfix] exit uid={musicInfo?.uid ?? "(null)"}, currentSelectedUid={CustomPlaySession.Current.SelectedMusicUid}, shouldApply={CustomPlaySession.Current.ShouldApplyExperimentChart}");
            string musicText = __instance.musicNameTitle != null ? __instance.musicNameTitle.text : "(null)";
            string artistText = __instance.artistNameTitle != null ? __instance.artistNameTitle.text : "(null)";

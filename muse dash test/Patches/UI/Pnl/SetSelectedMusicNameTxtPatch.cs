    ...
    internal static class SetSelectedMusicNameTxtPatchHelper
    {
        public static void Apply(Il2Cpp.SetSelectedMusicNameTxt instance, string source)
        {
            try
            {
                ...
                // 현재 곡은 LastKnownMusicUid 한 곳에서 해석합니다(세션 선택 → PnlStage → 마지막 클릭).
                // 마지막 클릭은 셀을 누를 때만 갱신되는 stale 값이라 맨 뒤에 둡니다. 예전 로직은
                // 앞의 값이 순정곡이면 이 stale 값으로 되돌아가, 커스텀곡을 본 직후 순정곡 준비화면에
                // 직전 커스텀 제목이 잔상으로 남는 버그가 있었습니다. 순정곡이면 아래에서 바로 끝납니다.

    ...
    internal partial class CustomTagPatch
    {
        ...
        internal class MusicInfo_albumUidName_Patch
        {
            ...
            private static bool Prefix(MusicInfo __instance, ref string __result)
            {
                ...
                {
                    _logged = true;
                    ModLogger.Verbose($"[CustomTagRegistry] 가상 곡 앨범 UID 응답: uid={__instance.uid} → '{__result}' (이 로그는 1회만 표시됩니다)");
                }
                return false;
            }
        }
        ...
        internal class MusicInfo_albumIndex_Patch
        {
            ...
            private static bool Prefix(MusicInfo __instance, ref int __result)
            {
                ...
                {
                    _logged = true;
                    ModLogger.Verbose($"[CustomTagRegistry] 가상 곡 앨범 인덱스 응답: uid={__instance.uid} → {__result} (이 로그는 1회만 표시됩니다)");
                }
                return false;
            }
        }
        ...
        internal class DBConfigLocalAlbums_GetLocalTitleByIndex_Patch
        {
            ...
            private static bool Prefix(int index, ref string __result)
            {
                ...
                {
                    _logged = true;
                    ModLogger.Verbose($"[CustomTagRegistry] 앨범 이름 현지화 응답: index={index} → '{__result}' (이 로그는 1회만 표시됩니다)");
                }
                return false;
            }
            ...
            private static bool Prefix(MusicInfo __instance, int language, ref LocalALBUMInfo __result)
            {
                if (__instance != null && CustomContentIds.IsVirtualSong(__instance.uid))
                {
                    if (HwaResourceManager.TryGetHwaPrimarySong(__instance.uid, out string title, out string artist, out _, out _, out _, out _, out _, out _, out _))
                    {
                        ...
                        __result = localInfo;
                        ModLogger.Verbose($"[MusicInfo.GetLocal Patch] 가상 곡 로컬 라이브러리 가로채기 성공: uid={__instance.uid}, title={title}, artist={artist}");
                        return false;
                    }
                }
                return true;
        ...
        internal class DBConfigLocalALBUM_GetLocalAlbumInfoByIndex_Patch
        {
            ...
            private static bool Prefix(DBConfigLocalALBUM __instance, int index, ref LocalALBUMInfo __result)
            {
                ...
                if (HwaResourceManager.TryGetHwaPrimarySong(currentUid, out string title, out string artist, out _, out _, out _, out _, out _, out _, out _))
                {
                    ...
                    localInfo.author = artist;
                    __result = localInfo;
                    ModLogger.Verbose($"[DBConfigLocalALBUM Patch] Index={index} 쿼리를 가상 곡 로컬 정보로 응답: uid={currentUid}, title={title}, artist={artist}");
                    return false;
                }
            ...
            private static void LogIndexDiagnostics(string uid, int index)
            {
                ...
                ModLogger.Verbose($"[DBConfigLocalALBUM 진단] 게임이 넘긴 index={index} vs 가상 곡 후보값 [{string.Join(", ", parts)}] " +

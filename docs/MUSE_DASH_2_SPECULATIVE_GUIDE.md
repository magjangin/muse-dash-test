...
3. **"BGM BGM 오디오와 BGA 비디오를 틀어주는 '네이티브 플레이어'가 어디에 붙어있는가?"**
   - **의미**: 게임 플레이 도중 음악과 배경 비디오를 출력하는 유니티 `AudioSource`와 `VideoPlayer` 컴포넌트가 **하이어라키(계층 구조) 상의 어떤 게임 오브젝트에 부착되는지** 파악하는 것입니다. 이 위치를 알아야 우리가 준비한 외부 `.ogg` 및 `.mp4` 파일을 로드해 플레이어에 이식할 수 있습니다.
...
- **목적**: 게임 엔진이 가상 UID(`999-0` 등)를 조회할 때 NullReference 예외로 폭사하지 않고, 우리가 준비한 복제본 데이터를 정상적으로 찾아가도록 우회 반환로를 가로채는 데 쓰입니다.
...
- 새로운 인스턴스를 무에서 `new`로 만들면 유니티 네이티브 메모리가 크래시될 수 있습니다. 1편에서 성공했던 **얇은 복제(Thin Clone, `MemberwiseClone()`)** 방식을 적용해 기존 순정 곡 정보를 복제한 뒤, 가상 곡 식별자(`999-0`)를 주입합니다.
- 가상 곡들이 등록된 앨범(`998-0` 등) 목록 또한 글로벌 리스트에 삽입합니다.
...
2편은 그래픽과 연출이 발전했을 것이므로 배경 영상(BGA) 지원 여부가 모딩의 퀄리티를 좌우할 것입니다.
...
- 2편에서도 카메라 구조는 크게 변하지 않을 것입니다. `Camera.main` 하위에 가상의 Quad 메쉬를 생성해 부모-자식 관계를 형성합니다.
- 카메라 해상도 종횡비(`aspect`)와 사이즈를 계산하여 꽉 채운 캔버스를 빌드하고, `VideoPlayer` 컴포넌트를 부착해 디스크의 `.mp4` 파일을 Material Override 모드로 출력합니다.
- 소팅 오더(`sortingOrder`)를 `-10` 이하로 내려 배경 이미지 뒤로 배치해 순정 UI의 가시성을 보존합니다.

---

## 🔒 Phase 5. 안전판 가동 (세이브 파일 오염 방지)

가상 곡(`999-0`)을 클리어했을 때 기록되는 점수와 결과 정보가 정식 세이브 파일 구조에 포함되면, 추후 모드를 껐을 때 세이브 데이터 손상(데이터 파싱 실패) 에러를 일으킬 수 있습니다.
...
- 저장하기 직전, 플레이 기록 딕셔너리에서 `999-` 및 `998-`로 시작하는 가상 곡의 플레이 이력 레코드 키들을 전부 걸러내고(Filter Out) 순정 데이터만 파일 시스템에 쓰도록 조치합니다.

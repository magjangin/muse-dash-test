...
| `8` | 샌드백/보스 멀티히트 | `020113`/`020114`, `boss_multihit_160` | 롱노트와 달리 중간 행(Middle)이 없으며 **단일 `MusicData` 슬롯 + `configData.length`** 구조로 동작합니다. 보스 샌드백은 `zz0113`/`zz0114`로 규격화되어 있으나, 일반 샌드백(몬스터 연타)은 씬/스테이지/디자인별 몬스터 프리팹 및 UID 후보(`010401`, `020401`, `030401` 등)가 너무 많아 단일 규격으로 특정하기 어렵습니다. |

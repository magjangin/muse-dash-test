...
프로토타입 전체가 `backup/local-main-20260809` 브랜치(및 그 히스토리의 커밋 `0220555`)에 보존되어 있습니다. 되살릴 때는:
...
둘 다 auto-property입니다 — `DBStageInfo.cs`에 `<sceneDialogEvents>k__BackingField` / `<sceneDialogDictionary>k__BackingField`가 존재합니다. **즉 백킹 필드에 직접 쓰는 것은 프로퍼티 setter를 호출하는 것과 완전히 동일하며, 생략되는 로직이 없습니다.** (setter는 `protected`라서 백킹 필드 경로를 쓰는 편이 편합니다.)

### `GameDialogArgs` 필드

IL2CPP struct이므로 인터롭에서 `sealed class GameDialogArgs : ValueType`로 생성되고, **`List`에 넣는 순간 값이 복사**됩니다. 따라서 필드를 모두 채운 뒤에 `Add`해야 합니다.
...
원본에 `GameDialogArgs.FixDialogDelay(argsDic, Decimal delay)`가 존재합니다. 즉 **스키마 자체가 대사 시간의 delay 보정을 전제**합니다.

현재 빌드에서는 `CallerCount(0)`이라 호출되지 않는 것으로 보이고, 그래서 `dialog.txt`의 raw 초를 그대로 쓴 프로토타입이 정상 동작했습니다.

**다만 이 모드는 가상 곡의 `DBStageInfo.delay` getter를 직접 오버라이드합니다** (`Patches/Diagnostics/OffsetHookPatches.cs`). 노트 타임라인이 `delay`를 먹는다면 노트만 밀리고 대사는 안 밀려 **정확히 delay만큼 어긋납니다.**
...
그리고 디투어가 꽂히는 **C++ 원본 메서드 포인터가 인터롭 어셈블리에 그대로 노출**되어 있습니다 (`DBStageInfo.cs`):
...
- [CUSTOM_CHART_GUIDE.md](../guides/CUSTOM_CHART_GUIDE.md) — 커스텀 차트 폴더 구조와 리소스 규약
- [MOD_SYSTEM_BLUEPRINT.md](../architecture/MOD_SYSTEM_BLUEPRINT.md) — 전체 훅 지점 목록
- [BMS_PARSING.md](../guides/BMS_PARSING.md) — 노트 시간 기준(대사 시간과 같은 기준)

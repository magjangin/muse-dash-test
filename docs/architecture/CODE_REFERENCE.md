...
* **고스트 노트의 실제 색**([GhostNoteTextureProbe.cs](../../muse%20dash%20test/Patches/Battle/Mechanics/GhostNoteTextureProbe.cs)로 측정). 슬롯 색·어태치먼트 틴트가 전부 `(1,1,1,1)`이라 컬러 타임라인의 `#FFFFFF`는 "텍스처를 안 건드림"이라는 뜻이고, 보이는 색은 전부 아틀라스 픽셀입니다. 팔레트는 세 가지뿐입니다 — `#FF3EB9`(몸통·손 채움), `#A1099F`(외곽선 겸 눈썹), `#FEE09C`(눈·입). 텍스처는 07 계열 공용 시트 `s07_atlas.png`(2048×1024)의 `0717/images_road/*`이고, `body`만 MeshAttachment이며 나머지 7개 슬롯은 RegionAttachment입니다.

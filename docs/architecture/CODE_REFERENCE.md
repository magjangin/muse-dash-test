...
* **접점 식별자는 슬롯 인덱스(0~9)**: Input System의 `touchId`는 단조 증가하여 값이 무한정 커지므로 게임에 넘기기 부적합합니다.
* **마우스 승격 차단 (`ShouldIgnoreMouse`)**: 터치 중 Windows가 첫 손가락을 마우스로도 승격시켜 보내므로, 두 경로를 다 받으면 한 번의 터치가 두 번 판정됩니다. 접점이 있는 동안과 그 직후 0.25초 마우스를 무시합니다. 승격이 터치보다 한두 프레임 늦게 도착하기 때문에 유예가 필요합니다.
* 프레임당 1회만 실제 조회하고 이후에는 캐시를 반환합니다.
...
* **`TaskStageTarget_GetAccuracy_Patch`, `GetTrueAccuracy_Patch` & `GetTrueAccuracyNew_Patch` (Postfix)**:
  ...
  * 정확도 갱신 시 분석 및 로깅을 위해 원본 및 오버라이드 변수 상태를 로그(`[APMod.Debug.Accuracy]`, Verbose 전용)로 기록합니다.

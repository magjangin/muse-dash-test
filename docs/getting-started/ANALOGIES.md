...
### 2.1 유니버설 래퍼 ➡️ "통역사" 🗣️
> **비유**: 낯선 언어로 된 매뉴얼(IL2CPP Interop 객체)을 매번 직접 해독하면, 게임이 업데이트될 때마다 표현이 바뀌어 다시 분석해야 합니다. 그래서 중간에 **통역사(`Il2CppWrapperBase`)**를 두었습니다. 모더는 익숙한 언어(C# 강타입 프로퍼티)로 요청하고, 통역사가 Interop 객체 필드/프로퍼티 값을 대신 가져옵니다.

**실제 의미**: 래퍼 계층이 리플렉션으로 Interop 객체 필드를 읽고 씁니다. `ModReflection`은 개발사가 변수명에 `m_`를 붙이거나 컴파일 시 백킹 필드(`_k__BackingField`)로 이름이 바뀌어도 대소문자를 무시하고 찾아내, 조회 실패로 인한 오류를 방지합니다.

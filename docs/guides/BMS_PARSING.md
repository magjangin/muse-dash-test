...
* 예: `011001_..._dt1.48.wav` ➡️ `Dt` = `1.48`초 (`showTick = tick - dt`)
* ⚠️ **주의**: `dt`는 노트가 화면 우측에서 스폰되어 오기까지의 **화면 선행 표시 시간(Display Time)**이며, 노트의 지속시간/길이가 아닙니다. 노트의 실제 지속시간은 **`configData.length`** 필드에 지정됩니다.
...
| :--- | :--- | :--- | :--- |
| **롱노트** | `xx == "02"` | **`3`** | `BmsNoteMatcher`에 의해 시작과 끝 쌍이 매칭되어 롱노트 체인 생성 (길이는 `configData.length`) |
| **톱니바퀴** | `xx == "03"` | **`2`** | 톱니 장애물 |
| **샌드백** | `xx == "04"` (단, `0004…` 제외) | **`8`** | 중간 행(Middle)이 존재하지 않는 **단일 슬롯 + `configData.length`** 구조의 멀티히트 노트 |
...
* **동작**: `BossAction`이 아래의 기준에 맞춰 자동으로 주입되고, 보스의 사전 전조 애니메이션 재생 시간을 확보하기 위해 **파일명에 `_dt` 선언이 없으면 `Dt`에 기본값 `0.7`초가 들어갑니다.** 파일명에 `_dt0.8`처럼 적어 두면 그 선언값을 그대로 씁니다(예전에는 선언값을 0.7로 덮어썼습니다).
* **보스 공격 액션 매핑 상세**:
...
- 헤더 `#BPM`은 차트의 **초기 상태**라서 리스트 맨 앞(0틱)에 놓이고, 같은 0틱에 적힌 채널 08 변경이 그 위에 적용됩니다.
  즉 **0마디에 적은 BPM 변경이 헤더 BPM을 이깁니다.** 예전에는 둘을 같은 목록에 섞어 `Source` 문자열로 정렬해서
  `BPM01`이 `default`보다 앞에 오고 헤더 BPM이 뒤에 다시 적용됐습니다(`#BPM 120` + `#BPM01:240` + `#00008:01`에서
  첫 노트가 1초가 아니라 2초로 계산됨).

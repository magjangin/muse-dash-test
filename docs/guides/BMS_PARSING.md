...
### 주석 (`//`, `;`)
* `//` 또는 `;`로 **시작하는 줄**은 통째로 무시합니다.
* **마디 줄**(`#00113:...`)과 **BPM 줄**(`#BPM`, `#BPMxx`)은 뒤에 붙은 주석을 떼고 읽습니다. 두 표식이 섞여 있으면 먼저 나온 쪽에서 자릅니다.
* **곡 정보 줄**(`#TITLE`, `#ARTIST`, `#WAVxx` 등)은 값을 자르지 않습니다. 값은 자유 텍스트라서 `Rock; Roll`, `AC//DC`처럼 표식이 그대로 들어갈 수 있기 때문입니다. 이런 줄에는 뒤에 주석을 붙이지 마세요.

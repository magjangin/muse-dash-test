...
# ============================================================================
#  모바일터치조작 : 마우스 클릭 및 터치스크린 입력을 모바일 터치(공중/지상)로 인식합니다.
#  모바일좌우모드 : true면 화면 좌우 분할(왼쪽:공중, 오른쪽:지상), false면 상하 분할(위:공중, 아래:지상)
#  모바일반전모드 : true면 터치/클릭 영역이 반대로(좌우/상하 반전) 바뀝니다.
#  모바일오토피버 : 모바일 자동 피버 활성화 여부입니다.

...
터치가 처음 들어올 때 **세션당 한 줄**만 기록합니다. 사용 중인 백엔드를 되짚기 위한 단서입니다. (v0.10.0부터 UMPC 환경에서는 `Error` 레벨로 자동 동작하여 이 안내 로그조차 억제되며, 예외/오류 시에만 기록됩니다.)

```text
[TouchInput] 터치스크린 입력을 InputSystem 백엔드로 처리합니다.
```

설정 UI 쪽은 일반 PC 환경에서 기본 로그가 유지됩니다. (UMPC 환경에서는 `Error` 레벨이 적용되어 숨겨집니다.)

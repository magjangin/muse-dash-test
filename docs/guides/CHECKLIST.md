...
- [ ] **곡별 offset이 게임 설정의 보정값을 보존하는가?**
      게임 보정 -100ms와 0ms에서 같은 곡을 비교하고 최종 offset 차이가 0.1초인지 확인합니다.
      (2026-09-11: 곡 offset으로 덮어써 사용자 보정이 무시됐습니다.)
      기존 절대값을 합산용으로 옮길 때는 `새 값 = 기존 값 - 맞출 때의 게임 보정값(초)`입니다.
      -100ms에서 맞춘 -0.05123은 0.04877로 옮깁니다. 다른 기준으로 맞춘 곡에 일괄 +0.1을 하지 않습니다.

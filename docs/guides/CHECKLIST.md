...
- [ ] **IL2CPP 패치 함정을 건드리지 않았는가?**
      ...
      (대상이 존재한다는 뜻일 뿐, 패치가 걸렸다는 뜻도 실행된다는 뜻도 아닙니다).
      반대로 **빨간불은 이제 믿어도 됩니다.** 예전에는 헬스체크가 클래스 레벨 어트리뷰트만 읽어서
      `[HarmonyPatch(typeof(X))]` + 메서드 레벨 `[HarmonyPatch("M")]` 형태를 `X.Normal 해석 실패`로
      오진했고, 맨 `[HarmonyPatch]` 클래스는 총계에서조차 빠졌습니다. 2026-08-28에 고쳤습니다.
...
- [ ] **기능을 끄는 게이트를 '쓰는 쪽'에만 달지 않았는가?**
      이 저장소는 기능을 끄는 창구가 셋입니다 — `config.txt`, MelonPreferences, 그리고 그 둘을 읽는
      `FeatureGuard`. 소비하는 쪽(그리기·적용)에만 게이트를 달고 **생산하는 쪽(Harmony 훅)을 그대로
      두면, 끈 상태에서 데이터만 쌓이고 비우는 코드는 영영 실행되지 않습니다.**
      정리 코드를 소비 쪽 early return **뒤에** 두었다면 특히 위험합니다.
      (2026-08-28: `JudgmentBar.RegisterHit`은 `GameTouchPlay.TouchResult` 훅에서 게이트 없이
      불렸는데, 만료 정리는 `DrawJudgmentBar`의 `if (!showBar) return;` **다음 줄**에만 있었습니다.
      판정바를 끄고 플레이하면 노트마다 `hitHistory`가 늘기만 했습니다.)
      목록을 늘리는 곳이 하나면, **줄이는 코드도 같은 곳에** 두는 편이 안전합니다.

- [ ] **게임 오브젝트를 이름 부분 일치로 고르지 않았는가?** 게임이 참조를 들고 있으면 그걸 쓰십시오.
      게임 업데이트가 비슷한 이름의 오브젝트를 새로 만들면 조용히 엉뚱한 쪽이 걸립니다.
      (2026-09-26: 게임 6.7.0이 `AudioManager.m_BgmOneShotSource`(GO `BGM_OneShot`)를 추가했고,
      이름에 "bgm"이 든 첫 소스를 고르던 배틀 BGM 주입이 그쪽을 집어 원곡과 커스텀 곡이 겹쳐 들렸습니다.
      `AudioManager.bgm`을 직접 쓰도록 고쳤습니다.) 게임이 업데이트되면 `SignatureDumper`로 새로 덤프해
      이전 덤프와 비교하는 것이 원인을 가장 빨리 찾는 길이었습니다.

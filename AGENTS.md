...
- If a source file grows beyond 600 lines, automatically refactor it into smaller, focused units before adding more substantial logic.
...
- Review `docs/guides/CHECKLIST.md` before finishing a code change and before publishing a release. Every item there records an actual past incident in this repo, not general advice — in particular, verifying the fallback path (no BMS/album → `0-0` fallback → actually enter battle) and checking that the build really landed in the game's `Mods` folder. When a new incident is worth preventing, add an item; when its rationale no longer holds, delete it.

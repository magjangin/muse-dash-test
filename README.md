...
* **진행 예정**: 로컬 `cover.png` 파일 실시간 디코딩을 통한 커스텀 곡 셀 앨범 아트 동적 주입
* **진행 예정**: 곡별 메타 마니페스트(`info.txt`) 내 `BgaOffsetMs` 항목을 통한 BGA 영상 싱크 미세 조정 기능
* **진행 예정**: 가상 곡 플레이 데이터(정확도, 스코어, 랭크)를 순정 세이브 손상 없이 로컬 전용 JSON 파일로 기록 및 영구 성과 연동
